...
class CircuitBoard(halfwidth: Int, halfHeight: Int) {
    ...
    fun drawWire(id: Char, wire: WireLine) {
        ...
        for (bend in wire.Line) {
            when (bend.Direction) {
                'R' -> {
                    for (i in IntRange(1, bend.Width)) {
                        currPos.x++
                        markField(id, currPos)
                    }
                }
                'D' -> {
                    for (i in IntRange(1, bend.Width)) {
...
class Layout(layoutDescription: String) {
    ...
    private fun parseLines(layout: String) {
        for (wire in layout.split("\n")) {
...
fun partOne() {
    ...
    val layout = Layout(puzzleInput)
    println("Loaded layout:\n$layout")

    val board = CircuitBoard(15000, 15000)
    ...
    println("Shorted distance to a crossing: ${board.getCrossingDistances().min()}")
}
